...
func (this *CodecClientJoinGame) Decode(reader io.Reader) (decode packet.Packet, err error) {
	...
	packetClientJoinGame.HasLastDeathPosition, err = packet.ReadBool(reader)
	if err != nil {
		return
	}
	if packetClientJoinGame.HasLastDeathPosition {
		packetClientJoinGame.LastDeathPositionWorld, err = packet.ReadString(reader)
		if err != nil {
			return
		}
		packetClientJoinGame.LastDeathPosition, err = packet.ReadUint64(reader)
		if err != nil {
			return
		}
	}
...
func (this *CodecClientJoinGame) Encode(writer io.Writer, encode packet.Packet) (err error) {
	...
	err = packet.WriteBool(writer, packetClientJoinGame.HasLastDeathPosition)
	if err != nil {
		return err
	}
	if packetClientJoinGame.HasLastDeathPosition {
		err = packet.WriteString(writer, packetClientJoinGame.LastDeathPositionWorld)
		if err != nil {
			return
		}
		err = packet.WriteUint64(writer, packetClientJoinGame.LastDeathPosition)
		if err != nil {
			return
		}
	}
