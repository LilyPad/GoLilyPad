...
func (this *CodecServerLoginEncryptResponse) Decode(reader io.Reader) (decode packet.Packet, err error) {
	...
	verifyTokenPresent, err := packet.ReadBool(reader)
	if err != nil {
		return
	}
	if verifyTokenPresent {
		...
		packetServerLoginEncryptResponse.Salt, err = packet.ReadInt64(reader)
		if err != nil {
			return
		}
		...
		if signatureLength < 0 {
			err = errors.New(fmt.Sprintf("Decode, Salt signature length is below zero: %d", signatureLength))
			return
		}
		if signatureLength > 65535 {
			err = errors.New(fmt.Sprintf("Decode, Salt signature length is above maximum: %d", signatureLength))
			return
		}
		packetServerLoginEncryptResponse.Signature = make([]byte, signatureLength)
		_, err = reader.Read(packetServerLoginEncryptResponse.Signature)
		if err != nil {
			return
		}
	}
...
func (this *CodecServerLoginEncryptResponse) Encode(writer io.Writer, encode packet.Packet) (err error) {
	...
	if packetServerLoginEncryptResponse.VerifyToken == nil {
		err = packet.WriteBool(writer, false)
		if err != nil {
			return
		}
		err = packet.WriteInt64(writer, packetServerLoginEncryptResponse.Salt)
		if err != nil {
			return
		}
		err = packet.WriteVarInt(writer, len(packetServerLoginEncryptResponse.Signature))
		if err != nil {
			return
		}
		_, err = writer.Write(packetServerLoginEncryptResponse.Signature)
	} else {
