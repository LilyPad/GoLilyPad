...
func ReadNbtNamed(reader io.Reader) (nbt Nbt, name string, err error) {
	tagId, err := packet.ReadUint8(reader)
	if err != nil {
		return
	}
	tag := NbtTag(tagId)
...
func readNbtString(reader io.Reader) (val string, err error) {
	length, err := packet.ReadUint16(reader)
	if err != nil {
		return
	}
...
func readNbtTagPayload(reader io.Reader, tag NbtTag, depth int) (val interface{}, err error) {
	switch tag {
	case NbtTagInt8:
		val, err = packet.ReadInt8(reader)
	case NbtTagInt16:
		val, err = packet.ReadInt16(reader)
	case NbtTagInt32:
		val, err = packet.ReadInt32(reader)
	case NbtTagInt64:
		val, err = packet.ReadInt64(reader)
	case NbtTagFloat32:
		val, err = packet.ReadFloat32(reader)
	case NbtTagFloat64:
		val, err = packet.ReadFloat64(reader)
	case NbtTagByteArray:
		var arrayLength uint32
		arrayLength, err = packet.ReadUint32(reader)
		if err != nil {
			return
		}
	...
	case NbtTagList:
		...
		var listTagId byte
		listTagId, err = packet.ReadUint8(reader)
		if err != nil {
			return
		}
		listTag := NbtTag(listTagId)
		var listLength uint32
		listLength, err = packet.ReadUint32(reader)
		if err != nil {
			return
		}
	...
	case NbtTagInt32Array:
		...
		for i := range array {
			array[i], err = packet.ReadInt32(reader)
			if err != nil {
				return
			}
		}
	...
	case NbtTagInt64Array:
		...
		for i := range array {
			array[i], err = packet.ReadInt64(reader)
			if err != nil {
				return
			}
		}
