...
func WriteNbtNamed(writer io.Writer, nbt Nbt, name string) (err error) {
	if nbt == nil {
		err = packet.WriteUint8(writer, byte(NbtTagEnd))
		return
	}
	err = packet.WriteUint8(writer, byte(NbtTagNbt))
	if err != nil {
		return
	}
...
func writeNbtString(writer io.Writer, val string) (err error) {
	err = packet.WriteUint16(writer, uint16(len(val)))
	if err != nil {
		return
	}
...
func writeNbtChild(writer io.Writer, nbt Nbt, depth int) (err error) {
	...
	for name, val := range nbt {
		...
		err = packet.WriteUint8(writer, byte(tag))
		if err != nil {
			return
		}
		err = writeNbtString(writer, name)
...
func writeNbtTagPayload(writer io.Writer, val interface{}, depth int) (err error) {
	...
	case bool:
		if val {
			err = packet.WriteInt8(writer, 1)
		} else {
			err = packet.WriteInt8(writer, 0)
		}
	case int8:
		err = packet.WriteInt8(writer, val)
	case int16:
		err = packet.WriteInt16(writer, val)
	case int32:
		err = packet.WriteInt32(writer, val)
	case int64:
		err = packet.WriteInt64(writer, val)
	case float32:
		err = packet.WriteFloat32(writer, val)
	case float64:
		err = packet.WriteFloat64(writer, val)
	case []byte:
		if len(val) > ByteArrayLimit {
			return ErrorLimit{"ByteArrayLimit", len(val), ByteArrayLimit}
		}
		err = packet.WriteUint32(writer, uint32(len(val)))
		if err != nil {
			return
		}
	...
	case []int32:
		...
		for _, arrayVal := range val {
			...
			err = packet.WriteInt32(writer, arrayVal)
		}
	case []int64:
		...
		for _, arrayVal := range val {
			...
			err = packet.WriteInt64(writer, arrayVal)
		}
	default:
		err = ErrorUnexpectedType
	}
...
func writeNbtListHeader(writer io.Writer, tag NbtTag, length int) (err error) {
	...
	err = packet.WriteUint32(writer, uint32(length))
	if err != nil {
		return
	}
	return
